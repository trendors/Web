...
export const campaignReducer = createReducer(
    ...
    on(CampaignActions.loadCampaignsSuccess, (state, { list }) => ({ ...state, isLoading:false, list })),
    ...
    on(CampaignActions.createCampaignSuccess, (state, { message }) => ({ ...state, isLoading: false })),
    on(CampaignActions.createCampaignFailure, (state, { error }) => ({ ...state, isLoading: false, error })),
);